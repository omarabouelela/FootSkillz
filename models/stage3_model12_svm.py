"""
Stage 3 - Model 12: Support Vector Machine (SVM)
-----------------------------------------------
- Loads features_balanced_windows.csv
- Encodes labels
- Train/test split (stratified)
- Pipeline: SimpleImputer -> StandardScaler -> SVC (RBF kernel)
...
# SVM hyperparameters (tune later if you like)
SVM_KERNEL = "rbf"
SVM_C = 1.0
SVM_GAMMA = "scale"
...
def main():
    ...
    # 5) Build pipeline: Imputer -> Scaler -> SVC
    # SVM is very sensitive to feature scaling, so StandardScaler is important.
    svm_clf = SVC(
        kernel=SVM_KERNEL,
        C=SVM_C,
        gamma=SVM_GAMMA,
        probability=False,   # set True if you later want predict_proba (slower)
        random_state=RANDOM_SEED,
    )
    ...
    print("Model details:")
    print("  Model type: SVC (sklearn.svm.SVC) with RBF kernel")
    print(f"  kernel: {SVM_KERNEL}")
    print(f"  C (regularization): {SVM_C}")
    print(f"  gamma: {SVM_GAMMA}")
    print("  No explicit learning_rate: SVM is solved via quadratic programming, not SGD.")
    print("  Training uses all samples in the optimization problem (no mini-batches).")
