"""
Stage 3 - Model 11: AdaBoost (Adaptive Boosting)
-----------------------------------------------
- Loads features_balanced_windows.csv
- Encodes labels
- Train/test split (stratified)
- Pipeline: SimpleImputer -> StandardScaler -> AdaBoostClassifier
...
# AdaBoost hyperparameters (can tune later)
...
def main():
    ...
    # 5) Build pipeline: Imputer -> Scaler -> AdaBoostClassifier
    ada_clf = AdaBoostClassifier(
        n_estimators=ADABOOST_N_ESTIMATORS,
        learning_rate=ADABOOST_LEARNING_RATE,
        random_state=RANDOM_SEED,
        # algorithm parameter omitted to stay compatible with your sklearn version
    )
    ...
    print("Model details:")
    print("  Model type: AdaBoostClassifier (sklearn.ensemble.AdaBoostClassifier)")
    print(f"  learning_rate: {ADABOOST_LEARNING_RATE}")
    print(f"  n_estimators: {ADABOOST_N_ESTIMATORS}")
    print("  Base estimator: decision stumps by default (depth-1 decision trees).")
    print("  AdaBoost reweights samples; learning_rate scales each estimator's contribution.")
