"""
Stage 3 - Model 2: Decision Tree Classifier
-------------------------------------------
- Loads features_balanced_windows.csv
- Encodes labels
- Train/test split (stratified)
- Pipeline: SimpleImputer -> (optional) StandardScaler -> DecisionTreeClassifier
...
def main():
    ...
    # 5) Build pipeline: Imputer -> Scaler (optional) -> Decision Tree
    # Note: Decision trees don't need scaling, but including it keeps
    # the preprocessing consistent across models and handles NaNs.
    ...
    pipeline = Pipeline(
        steps=[
            ...
            ("scaler", StandardScaler()),   # harmless for trees; keep for consistency
            ("clf", tree_clf),
        ]
    )
    ...
    print("Model details:")
    print("  Model type: DecisionTreeClassifier (sklearn.tree.DecisionTreeClassifier)")
    print("  No learning rate: tree is built in one pass by splitting rules.")
    print("  Training 'batch size': effectively uses the full dataset while building splits.")
