"""
Stage 3 - Model 6: XGBoost (XGBClassifier)
-----------------------------------------
- Loads features_balanced_windows.csv
- Encodes labels
- Train/test split (stratified)
- Pipeline: SimpleImputer -> StandardScaler -> XGBClassifier
...
# XGBoost hyperparameters (you can tune later)
...
def main():
    ...
    # 5) Build pipeline: Imputer -> Scaler -> XGBClassifier
    ...
    print("Model details:")
    print("  Model type: XGBClassifier (xgboost.XGBClassifier)")
    print(f"  learning_rate (eta): {XGB_LEARNING_RATE}")
    print(f"  n_estimators (trees): {XGB_N_ESTIMATORS}")
    print(f"  max_depth: {XGB_MAX_DEPTH}")
    print(f"  subsample: {XGB_SUBSAMPLE}")
    print(f"  colsample_bytree: {XGB_COLSAMPLE_BYTREE}")
    print("  Training is gradient-boosted trees; learning_rate controls step size for each boosting step.")
