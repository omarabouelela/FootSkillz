"""
Stage 3 - Model 10: Quadratic Discriminant Analysis (QDA)
--------------------------------------------------------
- Loads features_balanced_windows.csv
- Encodes labels
- Train/test split (stratified)
- Pipeline: SimpleImputer -> StandardScaler -> QuadraticDiscriminantAnalysis
- 5-fold cross-validation (accuracy)
- Reports training time, test metrics, inference time.
"""
...
def main():
    ...
    # 5) Build pipeline: Imputer -> Scaler -> QDA
    # reg_param helps avoid singular covariance issues; you can tune if needed.
    qda_clf = QuadraticDiscriminantAnalysis(
        reg_param=0.0  # set e.g. 0.1 if you get singular matrix warnings
    )
    ...
    print("Model details:")
    print("  Model type: QuadraticDiscriminantAnalysis (sklearn.discriminant_analysis.QuadraticDiscriminantAnalysis)")
    print("  No learning rate: QDA estimates class-specific Gaussian covariance matrices in closed form.")
    print("  Training uses all samples at once without SGD-style mini-batches.")
    print(f"  Inference batch size used for timing: {INFERENCE_BATCH_SIZE}")
    print("  Note: if you see 'singular matrix' warnings, increase reg_param (e.g., reg_param=0.1).")
