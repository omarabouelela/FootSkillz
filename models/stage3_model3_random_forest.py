"""
Stage 3 - Model 3: Random Forest Classifier
-------------------------------------------
...
def main():
    ...
    # 5) Build pipeline: Imputer -> Scaler -> Random Forest
    ...
    pipeline = Pipeline(
        steps=[
            ...
            ("scaler", StandardScaler()),   # not needed for RF, but consistent with other models
            ("clf", rf_clf),
        ]
    )
    ...
    print("Model details:")
    print("  Model type: RandomForestClassifier (sklearn.ensemble.RandomForestClassifier)")
    print("  No learning rate: forest is an ensemble of decision trees trained via bootstrapping.")
    print("  Training 'batch size': uses bootstrapped samples per tree, not SGD-style batches.")
